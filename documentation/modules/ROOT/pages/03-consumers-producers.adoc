...
Open a new terminal, _terminal 1_, and export environment variables for the Kafka instance bootstrap URL and the service account ID and Secret. Replace the placeholders with the values you copied before.
...
[source, bash-shell]
----
export $CLIENT_ID=<service account client id>
----

[.console-input]
[source, bash-shell]
----
export $CLIENT_SECRET=<service account client secret>
----
...
Open a new terminal, _terminal 2_, and export environment variables for the Kafka instance bootstrap URL and the service account ID and Secret. Replace the placeholders with the values you copied before.
...
Create a new file with the next content:
...
[source, bash-shell]
----
docker run -it --rm edenhill/kcat:1.7.0 kcat -b $BOOTSTRAP_SERVER -X security.protocol=SASL_SSL -X sasl.mechanisms=PLAIN -X sasl.username="$CLIENT_ID" -X sasl.password="$CLIENT_SECRET" -t songs -P -l -K: initial_songs
----
--
====
